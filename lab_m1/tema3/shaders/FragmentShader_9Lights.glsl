...
void main()
{
    ...
    if (floor_mode == 1) {
        for (int i = 0; i < 9; i++) {
            color = color + PointLightContribution(light_position[i], object_color[i]);
        }
    }
...
vec3 DiscoLightContribution()
{
    ...
    float attenuation_factor = 0.75;
    vec3 light_direction = world_position - disco_pos;

    // texcoord = texture coord. used for sampling
    vec2 texcoord;
    ...
    // lightColor -> color of world_position
